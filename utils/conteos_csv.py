...
def cargar_y_filtrar_datos():
    ...
    df = pd.read_csv(file_path, skiprows=7, sep='|', encoding='latin-1', low_memory=False)
    df1 = pd.read_csv(file_path, skiprows=5, sep='|', encoding='latin-1', nrows=1, header=None,
...
def calcular_data_values(df, df1):
    ...
    calculo_ln_actas_computadas = df['LISTA_NOMINAL'].astype(int).sum()
